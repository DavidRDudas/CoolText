...
- **🎧 Listen** — natural read-aloud (Web Speech API) with karaoke-style live word highlighting and auto-scroll. Click any word to start from there.
- **⚡ Focus mode** — RSVP speed reading: one word at a time, anchored on its optimal recognition point, from 100 to 1000 wpm, with smart pauses at commas and sentence ends.
- **Classic reader** — clean typography (serif or sans), adjustable text size, dark & light themes, reading progress bar.
- **🔒 Private** — everything is parsed in your browser. No server, no upload, no tracking.
...
- `app.js` — document parsing, reader rendering, speech engine, RSVP engine
